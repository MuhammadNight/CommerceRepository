...
const CreateSmartwatch = () => {
    ...
    const handleSubmit = (values, { resetForm }) => {
        ...
        dispatch(createProductData(formattedValues))
            .unwrap()
            .then(() => {
                setSuccessMessage('Smartwatch added successfully!');
                resetForm();
                dispatch(toggleModalAlert());
            })
            .catch((error) => {
                setErrorMessage(`Failed to add smartwatch: ${error.message}`);
            });
    };

    return (
        <ModalAlert title="Create Smartwatch">
            <Formik
            ...
            >
                {({ isSubmitting }) => (
                    <Form className="p-4 h-auto overflow-y-auto space-y-6">
                        <h2 className="text-xl font-semibold mb-4">Add new Smartwatch</h2>
